...
public:
    ...
    Graph(uint32_t v, uint32_t e) : V{v}, E{e},num_components{0}{
        reps.reserve(V);
        for(uint32_t i = 0; i < V; ++i){
            reps.push_back(i);
        }   //start by adding every vertice to the reps vector
        for(uint32_t i = 0; i < E; ++i){
            uint32_t rep;
            uint32_t follower;
            cin >> rep >> follower;
            union_set(rep,follower);
        }   //then set every edge 
    ...
    uint32_t find_set(uint32_t v) {
        uint32_t paths_start = v;
        while(reps[v] != v){
            v = reps[v];
        } //while we are not the ultimate repesentative, go to our next representative

        while(reps[paths_start] != v){
            uint32_t temp = reps[paths_start];
            reps[paths_start] = v;
            paths_start = temp;
        }   //path compression
        return v;
    ...
    uint32_t count_components() {
        for(uint32_t i = 0; i < reps.size(); ++i){
            if(reps[i] == i){
                ++num_components;
            }   //if your index equals your value than you must be the ultimate rep of some graph
        }
